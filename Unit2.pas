...
type
  ...
  private
    ...
    function IsLegalMove(piece: string; fromRow, fromCol, toRow, toCol: Integer; skipSafetyCheck: Boolean = False): Boolean;
    ...
    function IsKingInCheck(player: Char; skipSafety: Boolean = False): Boolean;
...
begin
 ...
 function TForm2.IsLegalMove(piece: string; fromRow, fromCol, toRow, toCol: Integer; skipSafetyCheck: Boolean = False): Boolean;
var
  dRow, dCol: Integer;
begin
  dRow := toRow - fromRow;
...
begin
  ...
  for r := 0 to 7 do
    for c := 0 to 7 do
    begin
      if IsLegalMove(piece, row, col, r, c, skipSafetyCheck) then
      begin
        move := Point(c, r);
        moves.Add(move);
      end;
...
function TForm2.IsKingInCheck(player: Char; skipSafety: Boolean = False): Boolean;
var
  row, col, r, c: Integer;
  kingPiece, piece: string;
  moves: TList<TPoint>;
  move: TPoint;
begin
  Result := False;
  ...
  for row := 0 to 7 do
    ...
    begin
      ...
      begin
        moves := GenerateLegalMoves(piece, row, col, True); // ✅ skipSafetyCheck
